...
public class LinearAssignmentSolverMTJ {

	public LinearAssignmentSolverMTJ() {
	}
	 ...
	 */
	public void solve(int size, FlexCompRowMatrix cost, int[] col_mate, int[] row_mate) {
		...
		// Do heuristic
		SparseVector row = cost.getRow(0);
		Iterator<VectorEntry> iter = row.iterator();
		VectorEntry ve;
...
//		}
		...
		for (k = 0; k < m; k++) {
			try {
				...
				row_dec[k] = s;
				for (l = 0; l < n; l++)
					if ((s == cost.get(k, l)) && (row_mate[l] < 0)) {
						col_mate[k] = l;
						row_mate[l] = k;
...
//									}
		...
		for (k = 0; k < m; ++k) {
			...
			while (iter.hasNext()) {
				ve = iter.next();
				l = ve.index();
				cost.set(k, l, (double) (ve.get() - row_dec[k] + col_inc[l]));
			}
//			for (l = 0; l < n; ++l) {
