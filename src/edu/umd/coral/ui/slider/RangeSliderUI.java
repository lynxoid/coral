...
class RangeSliderUI extends BasicSliderUI {
    ...
    public void paintTrack(Graphics g) {
        ...
        if (slider.getOrientation() == JSlider.HORIZONTAL) {
            ...
            // Draw selected range.
            if ( ((RangeSlider)slider).isDrawGradient()) {
            	// prepare a gradient
            	GradientPaint gradient = new GradientPaint(
            			lowerX - trackBounds.x, 
            			0, 
            			((RangeSlider)slider).getStartColor(), 
            			upperX - trackBounds.x, 
            			3, 
            			((RangeSlider)slider).getEndColor());
            	// set gradient on graphics for drawing
